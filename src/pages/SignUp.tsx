...
const SignUp: React.FC<SignUpProps> = ({ isModal, onSwitchAuthType }) => {
  ...
  const formContent = (
    <div className="space-y-6">
      <div className="space-y-2 text-center">
        ...
        <p className="text-black/60">Create an account to access exclusive offers and track your orders</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        <Button 
          variant="outline" 
          type="button"
          className="w-full flex items-center justify-center space-x-2 border-black/20 hover:bg-black/5"
          onClick={() => handleSocialSignUp('Google')}
        >
          <svg viewBox="0 0 24 24" width="16" height="16" xmlns="http://www.w3.org/2000/svg" className="fill-black">
      ...
      <div className="relative my-8">
        ...
        <div className="relative flex justify-center">
          <span className="bg-white px-4 text-sm text-black/60">Or sign up with email</span>
        </div>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Button type="submit" className="w-full mt-6 bg-black hover:bg-black/80 text-white" disabled={form.formState.isSubmitting}>
  ...
  if (!isModal) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-black via-neutral-800 to-black flex items-center justify-center">
        <div className="container max-w-md mx-auto py-10 px-4">
          <div className="mb-6">
            <Link to="/" className="inline-flex items-center text-white hover:text-gray-300 transition">
